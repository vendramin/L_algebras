...
create_files := function(n)
  local f,x,s; 
  s := create_file(n);
  f := IO_File(Concatenation("dualBCK", String(n), ".eprime"), "w");
  for x in s do
    IO_WriteLine(f, x);
  od;
  IO_Flush(f);
...
keep_dualBCK := function(n, filename)
...
construct_dualBCK := function(n)
  ...
  LogTo();
  LogTo(Concatenation("dualBCK", String(n), ".log"));

  create_files(n);

  Print("Running savilerow. ");
  output := Concatenation("output", String(n));
  Exec(Concatenation(s, "dualBCK", String(n), ".eprime >", output));
  for x in keep_dualBCK(n, output) do 
    Add(t, x);
    m := m+1;
  od;
  ...
  Print("I constructed ", m, " dual BCK algebras in ", mytime, "ms (=", StringTime(mytime), ")\n");

  f := IO_File(Concatenation("dualBCK", String(n), ".g"), "w");
  IO_WriteLine(f, Concatenation("dualBCK", String(n), " := ["));
  for x in t do

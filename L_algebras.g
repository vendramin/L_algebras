...
create_files := function(n)
  local f,x,s; 
  s := create_file(n);
  f := IO_File(Concatenation("size", String(n), ".eprime"), "w");
  for x in s do
    IO_WriteLine(f, x);
  od;
  IO_Flush(f);
...
keep_Lalgebras := function(n, filename)
...
construct := function(n)
  ...
  Exec(Concatenation(s, "size", String(n), ".eprime >", output));
  for x in keep_Lalgebras(n, output) do 
  ...
  Print("I constructed ", m, " L-algebras in ", mytime, "ms (=", StringTime(mytime), ")\n");

  f := IO_File(Concatenation("size", String(n), ".g"), "w");
  IO_WriteLine(f, Concatenation("size", String(n), " := ["));
  for x in t do

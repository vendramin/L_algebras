...
create_files := function(n)
  local f,x,s; 
  s := create_file(n);
  f := IO_File(Concatenation("CL", String(n), ".eprime"), "w");
  for x in s do
    IO_WriteLine(f, x);
  od;
  IO_Flush(f);
...
construct_CL := function(n)
  ...
  LogTo();
  LogTo(Concatenation("CL", String(n), ".log"));

  create_files(n);

  Print("Running savilerow. ");
  output := Concatenation("output", String(n));
  Exec(Concatenation(s, "CL", String(n), ".eprime >", output));
  for x in keep_CL(n, output) do 
  ...
  Print("I constructed ", m, " CL-algebras in ", mytime, "ms (=", StringTime(mytime), ")\n");

  f := IO_File(Concatenation("CL", String(n), ".g"), "w");
  IO_WriteLine(f, Concatenation("CL", String(n), " := ["));
  for x in t do

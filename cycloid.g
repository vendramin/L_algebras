...
create_files := function(n)
  local f,x,s; 
  s := create_file(n);
  f := IO_File(Concatenation("cycloid", String(n), ".eprime"), "w");
  for x in s do
    IO_WriteLine(f, x);
  od;
  IO_Flush(f);
...
keep_cycloid := function(n, filename)
...
construct_cycloid := function(n)
  ...
  LogTo();
  LogTo(Concatenation("cycloid", String(n), ".log"));

  create_files(n);

  Print("Running savilerow. ");
  output := Concatenation("output", String(n));
  Exec(Concatenation(s, "cycloid", String(n), ".eprime >", output));
  for x in keep_cycloid(n, output) do 
    Add(t, x);
    m := m+1;
  od;
  ...
  Print("I constructed ", m, " cycloids in ", mytime, "ms (=", StringTime(mytime), ")\n");

  f := IO_File(Concatenation("cycloid", String(n), ".g"), "w");
  IO_WriteLine(f, Concatenation("cycloid", String(n), " := ["));
  for x in t do

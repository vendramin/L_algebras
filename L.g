...
construct_L := function(n)
  ...
  Exec(Concatenation(s, "L", String(n), ".eprime >", output));
  for x in keep_L(n, output) do 
  ...
  f := IO_File(Concatenation("L", String(n), ".g"), "w");
  IO_WriteLine(f, Concatenation("L", String(n), " := ["));
  for x in t do

create_file := function(n)
  ...
  "forAll x,y,z : int(1..n) .", 
  "  M[M[M[x,y],z],M[M[y,x],z]]=M[M[M[x,y],z],z],\n",
  "forAll x : int(1..n-1) .",
  "  forAll y: int(x+1..n) .",
  "    M[x,y]+M[y,x]<2*n,\n",
  ];
...
create_files := function(n)
  local f,x,s; 
  s := create_file(n);
  f := IO_File(Concatenation("semiregular", String(n), ".eprime"), "w");
  for x in s do
    IO_WriteLine(f, x);
  od;
  IO_Flush(f);
...
keep_semiregular := function(n, filename)
...
construct_semiregular := function(n)
  ...
  LogTo();
  LogTo(Concatenation("semiregular", String(n), ".log"));

  create_files(n);

  Print("Running savilerow. ");
  output := Concatenation("output", String(n));
  Exec(Concatenation(s, "semiregular", String(n), ".eprime >", output));
  for x in keep_semiregular(n, output) do 
    #SymmetricGroup(n-1)) do
    Add(t, x);
    m := m+1;
  od;
  ...
  Print("I constructed ", m, " semiregular L-algebras in ", mytime, "ms (=", StringTime(mytime), ")\n");

  f := IO_File(Concatenation("semiregular", String(n), ".g"), "w");
  IO_WriteLine(f, Concatenation("semiregular", String(n), " := ["));
  for x in t do

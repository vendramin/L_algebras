...
create_files := function(n)
  local f,x,s; 
  s := create_file(n);
  f := IO_File(Concatenation("discrete", String(n), ".eprime"), "w");
  for x in s do
    IO_WriteLine(f, x);
  od;
  IO_Flush(f);
...
keep_discrete := function(n, filename)
...
construct_discrete := function(n)
  ...
  LogTo();
  LogTo(Concatenation("discrete", String(n), ".log"));

  create_files(n);

  Print("Running savilerow. ");
  output := Concatenation("output", String(n));
  Exec(Concatenation(s, "discrete", String(n), ".eprime >", output));
  for x in keep_discrete(n, output) do 
    Add(t, x);
    m := m+1;
  od;
  ...
  Print("I constructed ", m, " discrete L-algebras in ", mytime, "ms (=", StringTime(mytime), ")\n");

  f := IO_File(Concatenation("discrete", String(n), ".g"), "w");
  IO_WriteLine(f, Concatenation("discrete", String(n), " := ["));
  for x in t do
